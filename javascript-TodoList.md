...
<body>
  <header>
    ...
    <div class="input__container"><label for="todo-input" class="a11y-hidden">할일 추가하기</label><input id="todo-input"class = 'addInput'type="text" name ="todo" placeholder="What to do">
...
const createTodo = (content) => {
  ...
  const newTodo = {id: idList.length? Math.max(...idList)+1 : 1, content, completed:false}
  return newTodo;
}
//추가
...
const changeCheck = check=> {
  todos = todos.map(item => {
    return item.id ===check ? {...item, ...{completed: !item.completed}} : {...item};
  });
  $todoCheck = document.querySelectorAll('input[type=checkbox]')
  $deleteButton = document.querySelectorAll('.delete-button');
} 
